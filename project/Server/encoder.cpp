...
char sha256[256];
char* SHA_Table[MAX_CHUNK_NUM][4];
...
// void deduplication(unsigned char *buff, int chunk_num);
...
void cdc(unsigned char *buff, unsigned int buff_size)
{
    for(int i=16; i < (buff_size-16); i+=1){
        ...
        if((hash % MODULUS) == TARGET){
            ...
            SHA_Table[1][chunk_num-chunk_offset] = StrSHA256(cur_chunk,Last_Index-First_Index+1, sha256);
            // SHA_Table[1][chunk_num-chunk_offset] = (char*)'a';
            cout << SHA_Table[1][chunk_num-chunk_offset] << endl;
            sprintf(buffer,"%d",chunk_num);
            SHA_Table[0][chunk_num-chunk_offset] = buffer;
            sprintf(buffer,"%d",Last_Index);
            SHA_Table[2][chunk_num-chunk_offset] = buffer;
            sprintf(buffer,"%d",First_Index);
            SHA_Table[3][chunk_num-chunk_offset] = buffer;
        // //    deduplication(buff,chunk_num);
            cout << chunk_num << endl;
            chunk_num++;
...
//   if(!check){
//     char* chunk;
//     memcpy(&buff[strtol(SHA_Table[3][chunk_num-chunk_offset],&SHA_Table[3][chunk_num-chunk_offset],0)], &chunk, SHA_Table[2][chunk_num-chunk_offset]-SHA_Table[3][chunk_num-chunk_offset]+1);
